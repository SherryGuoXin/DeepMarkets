...
-- Report quarters represented by the complete source ZIP, including
-- accessions that were already supplied by daily EDGAR ingestion.
CREATE TABLE ETL_BATCH_REPORT_QUARTER (
    ETL_BATCH_ID INTEGER NOT NULL,
...
-- Optional incremental EDGAR ingestion provenance. These tables can be
-- dropped after removing their accession rows to restore the bulk-only schema.
CREATE TABLE DAILY_EDGAR_RUN (
...
CREATE TABLE DAILY_EDGAR_ACCESSION (
...
CREATE TABLE DAILY_EDGAR_PUBLICATION (
    ACCESSION_NUMBER VARCHAR2(25) PRIMARY KEY,
...
-- Request-ready feed for the Latest Filings API. It is refreshed by bulk ETL
-- and incrementally for manager/quarters affected by daily EDGAR publishing.
CREATE TABLE LATEST_FILING_FEED (
    ACCESSION_NUMBER VARCHAR2(25) PRIMARY KEY,
...
CREATE INDEX LATEST_FILING_FEED_DATE_IDX
    ON LATEST_FILING_FEED (FILING_DATE_ISO DESC, ACCESSION_NUMBER DESC);

CREATE INDEX LATEST_FILING_FEED_MANAGER_QUARTER_IDX
    ON LATEST_FILING_FEED (MANAGER_CIK, QUARTER_ID);

-- Request-ready materializations for a latest quarter that is still being
-- collected from daily EDGAR filings. Status metadata distinguishes this
-- preliminary snapshot from a batch-reconciled complete quarter.
CREATE TABLE DAILY_CIK_QUARTER_STATUS (
...
CREATE INDEX DAILY_CIK_HOLDING_QUARTER_VALUE_IDX
...
CREATE INDEX DAILY_CIK_HOLDING_QUARTER_CHANGE_IDX
    ON DAILY_CIK_HOLDING (QUARTER_ID, ABS(VALUE_CHANGE_USD) DESC);

CREATE INDEX DAILY_CIK_HOLDING_ACTIVITY_FILTER_IDX
    ON DAILY_CIK_HOLDING (
        QUARTER_ID, OPTION_TYPE, ACTION, ABS(VALUE_CHANGE_USD) DESC
    );

CREATE INDEX DAILY_CIK_HOLDING_ACTIVITY_ALL_IDX
    ON DAILY_CIK_HOLDING (
        QUARTER_ID, OPTION_TYPE, ABS(VALUE_CHANGE_USD) DESC
    );

CREATE TABLE DAILY_CIK_QUARTER_SUMMARY (
    MANAGER_CIK CHAR(10) NOT NULL,
    QUARTER_ID INTEGER NOT NULL,
...
CREATE INDEX DAILY_CIK_SUMMARY_QUARTER_VALUE_IDX
    ON DAILY_CIK_QUARTER_SUMMARY (QUARTER_ID, PORTFOLIO_VALUE_USD DESC);

CREATE TABLE DAILY_CIK_QUARTER_ACTIVITY (
    MANAGER_CIK CHAR(10) NOT NULL,
    QUARTER_ID INTEGER NOT NULL,
...
CREATE TABLE DAILY_CUSIP_QUARTER_SUMMARY (
    CUSIP CHAR(9) NOT NULL,
    QUARTER_ID INTEGER NOT NULL,
    ISSUER TEXT,
    TITLE_OF_CLASS TEXT,
...
CREATE INDEX DAILY_CUSIP_SUMMARY_QUARTER_VALUE_IDX
    ON DAILY_CUSIP_QUARTER_SUMMARY (QUARTER_ID, TOTAL_VALUE_USD DESC);

-- One row per security observed in the daily layer. This keeps global search
-- current without prematurely merging daily-only CUSIPs into bulk dimensions.
CREATE TABLE DAILY_CUSIP_IDENTITY (
...
CREATE TABLE DAILY_CUSIP_QUARTER_ACTIVITY (
    CUSIP CHAR(9) NOT NULL,
    QUARTER_ID INTEGER NOT NULL,
...
CREATE TABLE DAILY_CUSIP_OPTION_SUMMARY (
    CUSIP CHAR(9) NOT NULL,
    QUARTER_ID INTEGER NOT NULL,
    OPTION_TYPE TEXT NOT NULL,
    TOTAL_VALUE_USD INTEGER NOT NULL,
...
CREATE INDEX NORMALIZED_FILING_DATE_IDX
...
-- Effective raw-value unit for each filing. Before January 3, 2023 the SEC
-- instructed filers to report thousands, but some filings contain dollars.
-- The materialized decision prevents those filings from being multiplied
-- by 1,000 a second time while preserving evidence and manual review.
CREATE TABLE FILING_VALUE_SCALE_OVERRIDE (
    ACCESSION_NUMBER VARCHAR2(25) PRIMARY KEY,
...
CREATE TABLE FILING_VALUE_SCALE (
    ACCESSION_NUMBER VARCHAR2(25) PRIMARY KEY,
    VALUE_MULTIPLIER INTEGER NOT NULL CHECK (VALUE_MULTIPLIER IN (1, 1000)),
...
CREATE INDEX CANONICAL_FILING_LATEST_ACCESSION_IDX
